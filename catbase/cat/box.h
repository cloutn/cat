...
public:
	...
	Box(const scl::vector3& _min, const scl::vector3& _max) : m_min(_min), m_max(_max) { ensureMinMax(); }
	...
	scl::vector3			size			() const					{ return isValid() ? (m_max - m_min) : scl::vector3::zero(); }
	bool					isEmpty			() const					{ return isValid() ? (m_max == m_min) : true; }
	bool					isValid			() const					{ return (m_max.absolute() != scl::vector3::infinity()) && (m_min.absolute() != scl::vector3::infinity()); }

	void					set				(const scl::vector3& _min, const scl::vector3& _max) { m_min = _min; m_max = _max; ensureMinMax(); }
