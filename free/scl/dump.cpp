...
void			_dump_printf					(const wchar_t* format, ...)
{
	va_list arg;
	va_start(arg, format);
	wchar_t s[1024] = { 0 };
	vswprintf_s(s, sizeof(s) / sizeof(s[0]), format, arg);
	wprintf(s);
...
bool DbgHelpFunctions::loadMiniDump()
{
	...
	if (NULL == dll)
	{
		_dump_printf(L"dbghelp.dll not found!");
		return false;
	}
...
int dump_generate(_EXCEPTION_POINTERS *pExceptionInfo, const wchar_t* const filename)
{
	...
	{
		_dump_printf(L"load dbghelp.dll failed!");
	...
	{
		_dump_printf(L"can't create dump file!");
		return EXCEPTION_CONTINUE_SEARCH;
	}
	...
	{
		_dump_printf(L"write dump failed!");
	}
