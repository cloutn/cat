...
void log::set_quick_flush(LOG_LEVEL level, bool quick_flush)
{
	log_inst().m_levels[level].quick_flush = quick_flush;
}
...
void log::_log_handler(const log_header& header, const char* const s, int thread_id)
{
	...
	if (info.output & LOG_OUTPUT_FILE)
	{
		...
		if (info.filename.empty()) //全局文件
		{
			...
			__android_log_print(ANDROID_LOG_INFO, "scl", "scl lord game _log_handler exename111111 = %s", exename.c_str());

			if (exename.empty())
				get_android_files_dir(exename.c_str(), exename.capacity());

			__android_log_print(ANDROID_LOG_INFO, "scl", "scl lord game _log_handler exename222222 = %s", exename.c_str());
