...
template <int MAX_SIZE>
int scl::bitset<MAX_SIZE>::first_set_bit() const
{
	for (int i = 0;  i < FLAG_SIZE; ++i)
	{
		int n =  _get_first_set_bit(m_flag[i]);
		if (-1 != n)
			return i * 8 + n;
	}
	return -1;
...
template <int MAX_SIZE>
int scl::bitset<MAX_SIZE>::first_reset_bit() const
{
	for (int i = 0;  i < FLAG_SIZE; ++i)
	{
		int n =  _get_first_reset_bit(m_flag[i]);
		if (-1 != n)
			return i * 8 + n;
	}
	return -1;
