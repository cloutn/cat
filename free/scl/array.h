...
public:
	...
	template <class _Pr>
	int binary_search	(const T& elem, _Pr compare)	const	{ return scl::binary_search<T, _Pr>(m_array, 0, size() - 1, elem, compare); }
...
template<typename T>
int remove_dup(T* elems, const int elemCount)
{
	int end = 0;
	for (int i = 1; i < elemCount; ++i)
	{
		if (elems[i] != elems[end])
		{
			elems[end + 1] = elems[i];
			++end;
		}
